...
for (Stage in 1:(nrow(DefineStages) - 1)) {
  ...
  if (!(is.na(DurationType))) {
    ...
    if (DurationType == 3) {Duration <- ((-log(1 - runif(SampleSize))) ^ (1 / DurationShape)) * DurationScale}  # Set the preclinical distribution to be Weibull distributed   #Or you can use the code: Duration <- rweibull(SampleSize, shape = DurationShape, scale = DurationScale)
...
# Create Array to record outcomes with column names: we may update the StrategyName to make something descriptive of the strategies
strategies <-  paste(ScreenSchedule[, "StartAge"], ScreenSchedule[, "StopAge"], ScreenSchedule[, "Interval1"],
                        #ScreenSchedule[,"IntervalSwitchAge1"], ScreenSchedule[,"Interval2"], ScreenSchedule[,"IntervalSwitchAge2"],
                        #ScreenSchedule[,"Interval3"], ScreenSchedule[,"IntervalSwitchAge3"], ScreenSchedule[,"Interval4"], ScreenSchedule[,"IntervalSwitchAge1"],
                        #ScreenSchedule[,"ScreenSwitchAge"], ScreenSchedule[,"TestApplied1"], ScreenSchedule[,"TestApplied2"],
                        sep = "_")
#strategies <-  paste(StartAges, StopAges, Interval1s, sep = "_")

CostEffectivenessResults <- data.frame(
  StrategyName = c("NoScreening", strategies),
  ...
  ICER_L = as.numeric(NA),
  ICER_Q = as.numeric(NA)
)
...
# Record intermediate outcomes: costs
IntermediateOutcomes <- data.frame(
  StrategyName = c("NoScreening", strategies),
...
for (Strategy in c(1:length(strategies))){
  ...
  StartAge           <- ScreenSchedule[Strategy, "StartAge"]
  StopAge            <- ScreenSchedule[Strategy, "StopAge"]
  IntervalSwitchAge1 <- ScreenSchedule[Strategy, "IntervalSwitchAge1"]
  IntervalSwitchAge2 <- ScreenSchedule[Strategy, "IntervalSwitchAge2"]
  IntervalSwitchAge3 <- ScreenSchedule[Strategy, "IntervalSwitchAge3"]
  Interval1          <- ScreenSchedule[Strategy, "Interval1"]
  Interval2          <- ScreenSchedule[Strategy, "Interval2"]
  Interval3          <- ScreenSchedule[Strategy, "Interval3"]
  Interval4          <- ScreenSchedule[Strategy, "Interval4"]
  TestSwitchAge      <- ScreenSchedule[Strategy, "ScreenSwitchAge"]
  ScreenTest1        <- ScreenSchedule[Strategy, "TestApplied1"]
  ScreenTest2        <- ScreenSchedule[Strategy, "TestApplied2"]
  ...
  for (ScreenNumber in 1:nrow(ScreenCounts)) {
    ...
    ScreenCounts[ScreenNumber, c("N_Screens", "RealPostives", "TruePositives", "FalsePositives")] <- 
      cbind(
        length(which(ScreenEligible == 1)),
        length(AllPositives),
  ...
  write.table(ScreenCounts, paste("OutputFiles/LatestResults/IntermediateOutcomtes/ScreenCounts/", colnames(Input)[CurrentRun], "_", strategies[Strategy], ".txt", sep = ""),
    row.names = F,
    col.names = T,
    sep = '\t'
  )
  ...
  write.table(IntermediateOutcomes_AgeDist, paste("OutputFiles/LatestResults/IntermediateOutcomtes/AgeDistribution/", colnames(Input)[CurrentRun], "_", strategies[Strategy], ".txt", sep = ""),
    row.names = F,
    col.names = T,
    sep = '\t'
  )
  ...
  # Record intermediate outcomes
  IntermediateOutcomes[Strategy+1,-1] <- c( # The first column is the name of the strategy
    UD_ScreenCosts = UD_ScreenCosts,
  ...
  # Sum up the undiscounted and discounted costs
  CostEffectivenessResults[Strategy+1, c("UD_EffectsL", "UD_EffectsQ", "UD_Costs", "D_EffectsL", "D_EffectsQ", "D_Costs")] <-c(
    UD_Strategy_LYG, 
    UD_Strategy_QALY,
    UD_TotalCosts,
    D_Strategy_LYG,
...
SDfinder <- function(x, output){
  # Mark those subject to simple dominance as SD
  # This simply compares each strategy to all others to find cases of simple dominance
  if (sum((as.numeric(x[1]) <= AllEffects) * (as.numeric(x["D_Costs"]) > AllCosts)) >= 1){
    return("SD")
  }
  if (sum((as.numeric(x[1]) < AllEffects) * (as.numeric(x["D_Costs"]) >= AllCosts)) >= 1){
    return("SD")
...
CostEffectivenessResults[, "ICER_L"] <- apply(CostEffectivenessResults[, c('D_EffectsL','D_Costs')], 1, SDfinder)
AllEffects <- as.numeric(CostEffectivenessResults[, "D_EffectsQ"]) 
CostEffectivenessResults[, "ICER_Q"] <- apply(CostEffectivenessResults[, c('D_EffectsQ','D_Costs')], 1, SDfinder)

EDfinder <- function(CEresults, Effects, ICER, output){
  ...
  if (nrow(BoundarySet) == 1 | (length(unique(BoundarySet[, Effects])) == 1 & length(unique(BoundarySet[, "D_Costs"])) == 1)) {
    ...
    if(max(EfficientSet) < nrow(BoundarySet)){
      repeat{
        ...
        for (i in ((max(EfficientSet)+1):nrow(BoundarySet))){
          ...
          BoundarySet[i, ICER] <- round((BoundarySet[i, "D_Costs"] - BoundarySet[max(EfficientSet), "D_Costs"])/
...
BoundarySet_L <- EDfinder(CostEffectivenessResults, which(colnames(CostEffectivenessResults)=="D_EffectsL"), which(colnames(CostEffectivenessResults)=="ICER_L"))  
BoundarySet_Q <- EDfinder(CostEffectivenessResults, which(colnames(CostEffectivenessResults)=="D_EffectsQ"), which(colnames(CostEffectivenessResults)=="ICER_Q"))  

# Write in the ICERs of the efficient strategies
CostEffectivenessResults[match(BoundarySet_L[, "StrategyName"], CostEffectivenessResults[, "StrategyName"]), "ICER_L"] <- BoundarySet_L[, "ICER_L"]
CostEffectivenessResults[match(BoundarySet_Q[, "StrategyName"], CostEffectivenessResults[, "StrategyName"]), "ICER_Q"] <- BoundarySet_Q[, "ICER_Q"]
BoundarySet_L <- BoundarySet_L[which(!BoundarySet_L[, "ICER_L"] %in% "ED"), ]
BoundarySet_Q <- BoundarySet_Q[which(!BoundarySet_Q[, "ICER_Q"] %in% "ED"), ]
